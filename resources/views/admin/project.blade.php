...
<div class="pt-24 sm:pt-28 pb-8 sm:pb-12 px-4 sm:px-6">
    <div class="max-w-6xl mx-auto">
            ...
            <div>
                ...
                <div id="title-edit" class="hidden mt-2">
                    <form action="{{ route('admin.project.update', $project->id) }}" method="POST" class="flex gap-2">
                        @csrf
                        <input type="text" name="name" value="{{ $project->name }}" class="input flex-1 text-lg font-serif" required>
                        <button type="submit" class="btn btn-primary btn-md">Speichern</button>
                        <button type="button" onclick="cancelTitleEdit()" class="btn btn-ghost btn-md">Abbrechen</button>
                    </form>
                </div>
        ...
        <div class="card p-4 sm:p-5 mb-6">
            <h2 class="text-sm font-medium text-gray-700 mb-3 flex items-center gap-2">
            ...
            <div class="flex flex-col sm:flex-row items-start gap-4">
                ...
                <img src="/storage/projects/{{ $project->cover_image }}" alt="Cover" class="w-24 h-16 object-cover rounded-lg border border-gray-200 shrink-0">
                @else
                <div class="w-24 h-16 bg-gray-100 rounded-lg flex items-center justify-center text-gray-300 text-xs shrink-0">Kein Cover</div>
                @endif
                <div class="flex-1 w-full sm:w-auto">
                    <form action="{{ route('admin.project.update', $project->id) }}" method="POST" enctype="multipart/form-data" class="flex flex-wrap gap-2">
                        @csrf
                        <label class="btn btn-secondary btn-sm cursor-pointer">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>
                            <span class="ml-1">Cover wählen</span>
                            <input type="file" name="cover_image" accept="image/jpeg,image/png,image/webp" class="hidden" onchange="this.form.submit()">
                        </label>
                        @if($project->cover_image)
                        <button type="submit" name="remove_cover" value="1" class="btn btn-ghost btn-sm text-red-400 hover:text-red-600">Entfernen</button>
                        @endif
                    </form>
            ...
            <div class="lg:col-span-2 space-y-4">
                ...
                <div class="card p-4">
                    <h2 class="text-sm font-medium text-gray-700 mb-3 flex items-center gap-2">
                        ...
                        Fotos
                        @if($photos->count() > 0)
                        <span class="text-xs text-gray-400 font-normal">{{ $photos->count() }} Fotos</span>
                        @endif
                    </h2>
                    ...
                    <div class="mb-4">
                        <div class="flex flex-wrap items-center gap-2 mb-2">
                            <label class="text-xs text-gray-400">Ordner für Upload:</label>
                            <select id="upload-folder" class="input input-sm flex-1 min-w-[100px]">
                        ...
                        <div id="dropzone" class="border-2 border-dashed border-gray-200 rounded-2xl p-6 sm:p-8 text-center cursor-pointer hover:border-gold-300 hover:bg-gold-50/30 transition-all duration-200">
                    ...
                    {{-- Bulk Delete Bar --}}
                    <div id="bulk-bar" class="hidden mb-4 p-3 bg-red-50 border border-red-200 rounded-xl">
                        <div class="flex items-center justify-between flex-wrap gap-2">
                            ...
                            <div class="flex items-center gap-2">
                                ...
                                <button onclick="clearSelection()" class="text-xs text-red-500 hover:text-red-700 font-medium">Aufheben</button>
                                <button onclick="bulkDeletePhotos()" class="btn btn-danger-solid btn-sm">Löschen</button>
                            </div>
                        </div>
                    </div>
...
<script>
    ...
    function editTitle() {
        ...
        document.querySelector('h1[onclick="editTitle()"]').classList.add('hidden');
    }
    function cancelTitleEdit() {
        document.getElementById('title-edit').classList.add('hidden');
        document.querySelector('h1[onclick="editTitle()"]').classList.remove('hidden');
    }
    ...
    async function createFolder() {
        ...
        const data = await res.json();
        if (data.success) { showToast('Ordner erstellt'); location.reload(); }
    }
    ...
    async function deleteFolder(id, name) {
        ...
        if (data.success) {
            ...
            if (btn) { btn.style.transition = 'opacity 0.3s'; btn.style.opacity = '0'; setTimeout(() => btn.remove(), 300); }
    ...
    async function createShare() {
        ...
        if (!res.ok) { showToast('Fehler beim Erstellen', 'error'); return; }
        const data = await res.json();
        if (data.success) {
        ...
        } else { showToast(data.message || 'Fehler beim Erstellen', 'error'); }
    }
    ...
    function closeAdminLightbox() {
        ...
        if (window.__lbHistoryPushed) { window.__lbHistoryPushed = false; history.go(-2); }
    }
    ...
    function adminLbPrev() { adminLightboxIndex = (adminLightboxIndex - 1 + ADMIN_PHOTOS.length) % ADMIN_PHOTOS.length; updateLightbox(); }
    function adminLbNext() { adminLightboxIndex = (adminLightboxIndex + 1) % ADMIN_PHOTOS.length; updateLightbox(); }

    // Touch + mouse handlers for lightbox
    const adminPanContainer = document.getElementById('admin-lb-pan-container');
    adminPanContainer?.addEventListener('touchstart', (e) => {
        if (e.touches.length === 1) { adminIsDragging = true; adminDragStartX = e.touches[0].clientX; adminDragStartY = e.touches[0].clientY; adminDragStartPanX = adminPanX; adminDragStartPanY = adminPanY; lbTouchStartX = e.touches[0].clientX; lbTouchStartY = e.touches[0].clientY; }
        else if (e.touches.length === 2) { adminIsDragging = false; adminPinchStartDist = Math.hypot(e.touches[1].clientX - e.touches[0].clientX, e.touches[1].clientY - e.touches[0].clientY); adminPinchStartZoom = adminZoom; }
    }, { passive: true });
    adminPanContainer?.addEventListener('touchmove', (e) => {
        if (e.touches.length === 1 && adminIsDragging && adminZoom > 1) { e.preventDefault(); adminPanX = adminDragStartPanX + (e.touches[0].clientX - adminDragStartX); adminPanY = adminDragStartPanY + (e.touches[0].clientY - adminDragStartY); adminApplyTransform(); }
        else if (e.touches.length === 2) { e.preventDefault(); const dist = Math.hypot(e.touches[1].clientX - e.touches[0].clientX, e.touches[1].clientY - e.touches[0].clientY); if (adminPinchStartDist > 0) { const cx = (e.touches[0].clientX + e.touches[1].clientX) / 2; const cy = (e.touches[0].clientY + e.touches[1].clientY) / 2; adminZoomAt(cx, cy, Math.min(ADMIN_MAX_Z, Math.max(ADMIN_MIN_Z, adminPinchStartZoom * (dist / adminPinchStartDist)))); } }
    }, { passive: false });
    adminPanContainer?.addEventListener('touchend', (e) => {
        if (adminIsDragging && e.touches.length === 0) adminIsDragging = false;
        if (adminZoom <= 1 && e.changedTouches.length === 1) { const dx = e.changedTouches[0].clientX - lbTouchStartX; const dy = e.changedTouches[0].clientY - lbTouchStartY; if (Math.abs(dx) > Math.abs(dy) && Math.abs(dx) > 50) { if (dx < 0) adminLbNext(); else adminLbPrev(); } }
    }, { passive: true });
    ...
    function selectAllPhotos() { document.querySelectorAll('.photo-checkbox').forEach(cb => cb.checked = true); updateBulkBar(); }
    function clearSelection() { document.querySelectorAll('.photo-checkbox').forEach(cb => cb.checked = false); updateBulkBar(); }
    ...
    async function deleteAllPhotos() {
        ...
        if (data.success) {
            document.querySelectorAll('.photo-item').forEach(el => {
                el.style.transition = 'opacity 0.3s, transform 0.3s'; el.style.opacity = '0'; el.style.transform = 'scale(0.9)';
                setTimeout(() => el.remove(), 300);
            });
            showToast(data.deleted + ' Fotos gelöscht');
        } else { showToast('Fehler beim Löschen', 'error'); }
    ...
    // === DELETE (with DOM update, no reload) ===
    function deleteProject(id, name) {
        ...
        fetch('/admin/api/delete', { method: 'POST', body: fd })
            .then(r => r.json())
            .then(data => {
                if (data.success) { showToast('Projekt gelöscht'); setTimeout(() => location.href = '/admin', 500); }
                else { showToast('Fehler beim Löschen', 'error'); }
            })
        ...
        if (data.success) {
            ...
            if (el) {
                ...
                setTimeout(() => {
                    el.remove();
                    // Update photo count
                    const countEl = document.querySelector('h2 .text-xs.text-gray-400');
                    if (countEl) {
                        const match = countEl.textContent.match(/(\d+)/);
                        if (match) countEl.textContent = (parseInt(match[1]) - 1) + ' Fotos';
                    }
    ...
    async function deleteShare(id) {
        ...
        if (data.success) {
            ...
            if (el) {
                ...
                el.style.opacity = '0'; el.style.maxHeight = '0'; el.style.overflow = 'hidden'; el.style.margin = '0'; el.style.padding = '0';
                setTimeout(() => {
                    el.remove();
                    if (!document.querySelectorAll('.share-card').length) {
