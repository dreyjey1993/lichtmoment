...
<div class="pt-24 sm:pt-28 pb-8 sm:pb-12 px-4 sm:px-6">
    <div class="max-w-6xl mx-auto">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
            ...
            <a href="{{ route('admin.project.new') }}" class="inline-flex items-center gap-2 px-5 py-2.5 bg-gold-400 hover:bg-gold-500 text-white text-sm font-medium rounded-xl transition-colors shrink-0">
        ...
        <details class="sm:hidden mb-6 bg-white rounded-xl border border-gray-100 overflow-hidden">
            ...
            <div class="px-4 pb-4 border-t border-gray-100 pt-3">
                <form action="{{ route('admin.project.create') }}" method="POST" class="flex gap-2">
                    ...
                    <input type="text" name="name" placeholder="Projektname..." required
                           class="flex-1 min-w-0 px-3 py-2 border border-gray-200 rounded-lg text-sm focus:border-gold-400 outline-none">
                    <button type="submit" class="px-4 py-2 bg-gold-400 hover:bg-gold-500 text-white text-sm font-medium rounded-lg transition-colors shrink-0">Erstellen</button>
                </form>
            </div>
        </details>
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4 mb-8">
            <div class="bg-white rounded-xl border border-gray-100 p-4 sm:p-6 text-center">
        ...
        <div class="text-center py-20 bg-white rounded-xl border border-gray-100">
            ...
            <a href="{{ route('admin.project.new') }}" class="inline-flex items-center gap-2 px-5 py-2.5 bg-gold-400 hover:bg-gold-500 text-white text-sm font-medium rounded-xl transition-colors">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/></svg>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
            ...
            <div class="group bg-white rounded-xl border border-gray-100 overflow-hidden hover:shadow-lg transition-all duration-300 relative">
                ...
                <button onclick="deleteProject({{ $project->id }}, '{{ addslashes($project->name) }}')" class="absolute top-3 right-3 w-9 h-9 bg-red-500/90 hover:bg-red-600 text-white rounded-lg flex items-center justify-center transition-colors cursor-pointer shadow-sm" title="Projekt löschen">
